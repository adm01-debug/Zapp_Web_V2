...
## CP3 Boot          [x] sha=<pendente> (verificação Playwright ao vivo adiada p/ pós-Fase 5 — build só fecha depois que useThemePreset.ts/ThemeCustomizer.tsx forem reescritos; ver nota) · storage event: setItem=0 chamadas (spy, teste unitário) · migração v5→v6 testada (ThemeInitializer.test.tsx + theme-storage.test.ts, 22 testes) · troca dark↔light reaplica preset certo e atualiza cacheMode (etapa 38, testado)
...
- **CP3 (etapa 33/gate): screenshot `03-boot.png` e leitura ao vivo de `--background` antes do React montar** ficam pendentes até o build voltar a funcionar (Fase 5 fecha os 2 consumidores que ainda quebram o `tsc`/`vite build`). A lógica do boot (`index.html`) e a migração v5→v6 estão cobertas por unit tests (jsdom) que passam; falta só a confirmação end-to-end no bundle real.
