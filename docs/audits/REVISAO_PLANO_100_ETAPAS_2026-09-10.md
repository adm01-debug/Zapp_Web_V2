...
- A primeira tentativa de restaurar `20260910080000` ainda reteve alterações anexadas posteriormente; o dry-run canônico a bloqueou pelo hash do ledger antes de qualquer `apply`. A reconciliação corretiva restaura agora o corpo histórico de colunas de auditoria e simula a policy permissiva como pré-condição legada. A versão `20260910100000` só será considerada válida depois que outro dry-run comprovar seu hash remoto; nenhum novo número de migration será escolhido por inferência.
