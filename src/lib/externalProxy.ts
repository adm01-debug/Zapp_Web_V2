...
type ProxyParams = ProxySelectParams | ProxyMutationParams;
...
export async function queryExternalProxy<T = unknown>(params: ProxyParams): Promise<ProxyResponse<T>> {
  ...
  if (error) {
    ...
    // (404 Not Found, relay error, network failure) means the
    ...
    const typedError = error as { context?: { status?: number }; name?: string };
    ...
    const isProxyUnavailable =
      ...
      typedError.name === 'FunctionsFetchError' ||
      typedError.name === 'FunctionsRelayError';

    if (isProxyUnavailable) {
      return { data: [], count: 0, notConfigured: true };
    }
