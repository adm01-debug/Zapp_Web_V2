...
const buildPreset = (p: PresetParams): ThemePreset => {
  ...
  const light: ThemeModeColors = {
    ...
    'primary-glow': `${gh} ${Math.min(s + 3, 100)}% ${l + 6}%`,
    secondary: `${sh} ${ss}% ${sl}%`,
  ...
  const dark: ThemeModeColors = {
    ...
    'secondary-foreground': '0 0% 100%',
    muted: `216 45% 14%`,
    'muted-foreground': `${h} 22% 66%`,
    accent: `${h} 96% 21%`,
    'accent-foreground': `${h} 100% 85%`,
    border: `217 40% 16%`,
    input: `216 56% 9%`,
    ring: `${h} ${s}% ${l}%`,
    xp: `${h} ${s}% ${l}%`,
    ...
    'sidebar-primary': `${h} ${s}% ${l}%`,
    'sidebar-primary-foreground': '0 0% 100%',
    'sidebar-accent': `${h} 96% 21%`,
    'sidebar-accent-foreground': `${h} 100% 85%`,
    ...
    'gradient-divider': `linear-gradient(90deg, transparent, hsl(${h} 40% 30% / 0.5), transparent)`,
    'shadow-glow-primary': `0 8px 24px -8px hsl(${h} ${s}% ${l}% / 0.45)`,
...
export const PRESETS: ThemePreset[] = [
  buildPreset({ id: 'corporate', name: 'Padrão', description: 'Azul navy premium', emoji: '💼', h: 217, s: 100, l: 54, gh: 217, sh: 213, ss: 94, sl: 62 }),
...
/** Bump quando a paleta base mudar: invalida cssVarsCache antigo gravado no localStorage (index.html + ThemeInitializer). */
