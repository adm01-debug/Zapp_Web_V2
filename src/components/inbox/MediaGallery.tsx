...
interface MediaGalleryProps {
  ...
  /** Renderiza embutido no layout (sem Dialog modal) */
  inline?: boolean;
}
...
export function MediaGallery({ contactId, open = false, onOpenChange, inline = false }: MediaGalleryProps) {
  ...
  const body = (
    <>
      ...
      <ScrollArea className={cn('flex-1', inline ? 'max-h-[360px]' : 'min-h-[300px]')}>
        {isLoading ? (
          <div className={cn('grid gap-2 p-2', inline ? 'grid-cols-3' : 'grid-cols-4')}>{[...Array(8)].map((_, i) => <Skeleton key={i} className="aspect-square rounded-lg" />)}</div>
        ) : filteredItems.length === 0 ? (
          <GenericEmptyState icon={Image} title="Sem mídias" description="Nenhuma mídia encontrada nesta conversa" className="py-8" />
        ) : viewMode === 'grid' ? (
          <div className={cn('grid gap-2 p-2', inline ? 'grid-cols-3' : 'grid-cols-4')}>
