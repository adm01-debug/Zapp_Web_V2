...
interface MediaGalleryProps {
  contactId: string;
  /** Modo modal (padrao). Ignorado quando inline. */
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  /** Renderiza o conteudo embutido, sem Dialog (uso no painel de detalhes do contato). */
  inline?: boolean;
}

export function MediaGallery({ contactId, open = false, onOpenChange, inline = false }: MediaGalleryProps) {
  ...
  const preview = <MediaPreviewDialog item={previewItem} open={!!previewItem} onOpenChange={(o) => !o && setPreviewItem(null)} />;

  if (inline) {
    return (
      <div className="flex flex-col gap-2 max-h-[420px]">
        <div className="flex items-center gap-2 text-sm font-medium">
          <Grid3X3 className="w-4 h-4" />Galeria de Mídia<Badge variant="secondary">{counts.all} itens</Badge>
        </div>
        {body}
        {preview}
      </div>
