                  ...
                  {variants.map((v) => (
                    <div key={v.id} className="rounded-xl border border-border/60 bg-input/20 p-2 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-bold text-primary flex items-center gap-1">Variante {v.label} · <input type="number" min={1} max={100} value={v.weight} onChange={(e) => setVariants(vs => vs.map(x => x.id === v.id ? {...x, weight: Math.max(1, Math.min(100, parseInt(e.target.value)||1))} : x))} onBlur={async (e) => { const w = Math.max(1, Math.min(100, parseInt(e.target.value)||1)); if (w === v.weight) return; setSavingVariant(true); try { await saveVariant(activeTemplateId!, {...v, weight: w}); } finally { setSavingVariant(false); } }} className="w-9 text-center bg-transparent border-b border-primary/40 outline-none text-[11px] font-bold text-primary" />%</span>
