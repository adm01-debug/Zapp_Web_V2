...
export function TalkXTemplateEditor({ templates, isLoading, editing, onClose }: Props) {
  const { createTemplate, updateTemplate, duplicateTemplate, testTemplate, fetchVersionHistory, saveVersionSnapshot, fetchVariants, saveVariant, deleteVariant } = useTalkXTemplates();
        ...
        <div className="rounded-2xl bg-card border border-border/70 p-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              }} className="h-7 px-2 rounded-md text-[11px] font-medium border border-border/60 bg-input/40 hover:bg-muted/50">{showVariants ? 'Ocultar' : (variants.length > 0 ? `${variants.length} variante(s)` : 'Adicionar variante')}</button>
              {showVariants && (
                <div className="space-y-2 mt-1">
                  {variants.map((v) => (
                    <div key={v.id} className="rounded-xl border border-border/60 bg-input/20 p-2 space-y-1">
                      ...
                      <textarea
                        ...
                        onBlur={async () => { setSavingVariant(true); try { await saveVariant(activeTemplateId!, v); } finally { setSavingVariant(false); } }}
                  ...
                  {variants.length < 3 && (
                    <button type="button" disabled={savingVariant || !activeTemplateId} onClick={async () => {
                      const nextLabel = (['A', 'B', 'C'] as const).find(l => !variants.find(v => v.label === l))!;
                      const totalWeight = variants.reduce((s, v) => s + v.weight, 0);
                      const newWeight = Math.max(10, 100 - totalWeight);
                      setSavingVariant(true);
                      try {
                        await saveVariant(activeTemplateId!, { template_id: activeTemplateId!, label: nextLabel, content: eContent, media_url: null, media_type: null, weight: newWeight });
                        const vs = await fetchVariants(activeTemplateId!);
                        setVariants(vs);
                      } finally { setSavingVariant(false); }
                    }} className="w-full h-7 rounded-lg border border-dashed border-primary/40 text-[11px] text-primary hover:bg-primary/5 disabled:opacity-50">+ Adicionar variante {(['A','B','C']).find(l => !variants.find(v => v.label === l))}</button>
                  )}
                  {variants.length > 0 && (
