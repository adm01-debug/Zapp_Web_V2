...
--
-- RESTAURADO em 2026-08-30 (validacao exaustiva — gap G-11): o commit 7e42fc2b
-- substituiu o predicado evidence-gated abaixo por um rename incondicional
-- (UPDATE ... WHERE version='20260829020000'), o que quebrava o replay limpo
-- detectado por scripts/db-audit/check-reconcile-ledger-drift.test.sh. A logica
-- original do commit 45dd22e4 foi restaurada integralmente, preservando os
-- cinco gates obrigatorios:
--   1. nome anterior esperado ('mcp_exec_functions_harden');
--   2. cardinalidade exata do array (1 statement);
--   3. assinatura estrutural de reassign_absent_agents;
--   4. exclusao explicita de mcp_exec/mcp_exec_many;
--   5. SHA-256 domain-separated exato do array statements.
-- A migration ja esta aplicada no banco oficial; esta restauracao altera apenas
-- o comportamento de replay/fresh-install. Nenhuma mutacao em producao.
