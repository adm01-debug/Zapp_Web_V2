-- B3 FIX: search_contacts sem tiebreaker determinístico no ORDER BY
-- Quando dois contatos têm o mesmo nome, a ordem entre eles é arbitrária e pode
-- variar entre páginas, duplicando ou omitindo registros na paginação.
-- Fix: adicionar c.id ASC como critério final de desempate (UUID é único e estável).
