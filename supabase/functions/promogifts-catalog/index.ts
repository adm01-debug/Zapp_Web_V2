...
async function promogiftsCatalogHandler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (action === "list_products") {
      ...
      // Resolvido uma unica vez (fora do builder) pra poder reusar o mesmo
      // filtro na query principal e no fallback de contagem do PGRST103
      // abaixo, sem repetir a subconsulta.
      ...
      const buildQuery = (selectExpr: string, head = false) => {
        let q = extClient.from("products").select(selectExpr, { count: "exact", head });
      ...
      const query = buildQuery((compact ? PRODUCT_FIELDS_COMPACT : PRODUCT_FIELDS) as string)
      ...
      if (error) {
        if (error.code === "PGRST103") {
          // offset alem do total de linhas que o filtro atual retorna (ex.:
          // filtro reduziu o resultado, ou o client pediu uma pagina que ja
          // nao existe mais) -- o PostgREST responde 416/PGRST103 "Requested
          // range not satisfiable" em vez de uma pagina vazia. Reproduzido ao
          // vivo: mesmo filtro com offset=0 -> 200 total:0; com offset>0 ->
          // 503 antes deste fix (e o mesmo vale sem filtro nenhum, com um
          // offset maior que o total real). Nao e falha do catalogo externo;
          // refaz como count-only (head, sem range) pra devolver o total real.
          const { count: total } = await buildQuery("id", true);
